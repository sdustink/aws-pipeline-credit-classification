...
class DataPreprocessor:
    ...
    def clean(self):
        ...
        df["Age"] = (
            df["Age"].fillna("0").str.extract(r"(\d+)").astype(float).astype(int)
        )
        df["Num_of_Loan"] = (
            df["Num_of_Loan"]
            .fillna("0")
        ...
        df["Amount_invested_monthly"] = df["Amount_invested_monthly"].astype(
            str
        )
        ...
        df.drop(
            columns=["ID", "Customer_ID", "Month", "Name", "SSN", "Type_of_Loan"],
            inplace=True,
        )
...
class ModelTrainer:
    def __init__(self, preprocessor: ColumnTransformer, classifier):
...
class ModelEvaluator:
    ...
    def evaluate(self, X_test, y_test) -> float:
        ...
        report = classification_report(
            y_test, y_pred, target_names=self.target_encoder.classes_
        )
    ...
    def _export_artifacts(self):
        print("\nExporting Artifacts (Joblib & Tarball)...")
